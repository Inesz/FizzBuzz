...
class DivisorComputationTest {
    ...
    void shouldComputeForNumber(int number, String expectation) {
        assertEquals(expectation, new DivisorComputation().computeForNumber(number, BUZZFIZZ_CONDITIONS));
    }
    ...
    void shouldComputeForIntLimitAsNumber(int number) {
        assertEquals(Integer.toString(number), new DivisorComputation().computeForNumber(number, BUZZFIZZ_CONDITIONS));
    }
    ...
    void shouldComputeForNullOrEmptyConditions(Map<Integer, String> outputForDivisor) {
        assertEquals("1", new DivisorComputation().computeForNumber(1, outputForDivisor));
    }
}